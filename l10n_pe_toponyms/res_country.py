...
class country_province(osv.osv):
    ...
    _columns = {
            'state_id': fields.many2one('res.country.state', 'Pais', required=True),
            'name': fields.char('Nombre Provincia', size=64, required=True),
            'code': fields.char('Codigo', size=6, required=True),
            'district_ids': fields.one2many('res.country.district', 'province_id', 'Distritos'),
        }
    _sql_constraints = [
